...
public class CoalGolemItem extends Item {
    ...
    public void releaseUsing(ItemStack stack, Level level, LivingEntity livingEntity, int remainingTime) {
        ...
        if (stackInOtherHand.getItem() instanceof FlintAndSteelItem) {
            ...
            if (!level.isClientSide()) {
                stackInOtherHand.hurtAndBreak(1, player, p -> p.broadcastBreakEvent(otherHand));
            }
            shouldLight = true;
        ...
        var timeHeld = this.getUseDuration(stack) - remainingTime;
        ...
        for (var i = 0; i < throwCount; i++) {
            ...
            coalGolem.shootFromRotation(player, player.getXRot(), player.getYRot(), 0, 1.25f, 10);
    ...
    public static class DispenseItemBehaviour extends DefaultDispenseItemBehavior {
        ...
        protected ItemStack execute(BlockSource source, ItemStack stack) {
            ...
            var targetPos = source.getPos().relative(direction);
            ...
            coalGolem.shoot(direction.getStepX(), (float) direction.getStepY() + 0.1F, direction.getStepZ(), 1.25f, 2);
