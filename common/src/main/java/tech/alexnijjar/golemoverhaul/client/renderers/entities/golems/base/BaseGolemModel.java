...
public class BaseGolemModel<T extends BaseGolem> extends DefaultedEntityGeoModel<T> {
    ...
    public void setCustomAnimations(T animatable, long instanceId, AnimationState<T> animationState) {
        ...
        var headRotation = head.getChildBones().stream().findFirst();
        if (headRotation.isEmpty()) return;

        EntityModelData entityData = animationState.getData(DataTickets.ENTITY_MODEL_DATA);
        headRotation.get().setRotX(Mth.clamp(entityData.headPitch(), -maxHeadRotation, maxHeadRotation) * Mth.DEG_TO_RAD);
        headRotation.get().setRotY(Mth.clamp(entityData.netHeadYaw(), -maxHeadRotation, maxHeadRotation) * Mth.DEG_TO_RAD);
    }
